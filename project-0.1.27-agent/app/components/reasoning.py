...
AND of each adjacent pair of words (`_tokens` says why the pairs earn
their place), add-one smoothing, a log prior from the label balance, and
the decision is the label with the highest log-probability. It is fitted
at construction from `evals/golden.jsonl` -- the same file the harness
scores, which makes the golden score an in-sample number; the holdout the
engagement keeps is the out-of-sample one. Before this estimator, a
Bernoulli variant that charged every unseen token a per-label absence cost
drifted long inputs to the rarest class and recalled the commonest label
once in sixteen.
...
def _tokens(text: str) -> list[str]:
    """Words, and the adjacent pairs of them.

    An intent is carried by a phrase at least as often as by a word --
    "not working", "top up", "exchange rate", "show up" -- and a bag of
    single words cannot see one. It sees "card" and "working" in the same
    message as "card" and "arrive", and the labels that differ only by the
    phrase they share their words with collapse into each other: card
    delivery recalled 0.64, card acceptance 0.22, and a tenth of the file
    landed under the abstain threshold with no word in it deciding
    anything. Pairing adjacent tokens is the cheapest feature that
    separates them, and it separates them OUT of sample too, not only on
    the file the baseline is fitted on: on a 70/30 split of golden the
    held-out score went 70.8% -> 73.3% and the abstained share 16.5% ->
    11.8%, so this is a better classifier and not a memorised exam.

    The pairs are formed AFTER the stopwords go, so "when will my card
    arrive" pairs `card arrive` rather than `card will`: the removed word
    is the one carrying no signal, and bridging it is the point.
    """
    text = _strip_dictated_labels(text)
    words = [t for t in TOKEN.findall(text.lower()) if t not in STOPWORDS]
    # A space never appears in a single token (TOKEN admits none), so a
    # pair can never collide with a word.
    return words + [f"{first} {second}"
