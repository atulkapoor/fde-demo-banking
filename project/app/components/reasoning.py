...
Bayes over the client's own labelled pairs: counts per label of each word
and each adjacent pair of words (`_features`), Lidstone smoothing
(SMOOTHING -- the one number here worth reading), a log prior from the
label balance, and the decision is the label with the highest
log-probability. It is fitted at construction from `evals/golden.jsonl` --
the same file the harness scores, which makes the golden score an in-sample
number; the holdout the engagement keeps is the out-of-sample one. Before
this estimator, a Bernoulli variant that charged every unseen token a
per-label absence cost drifted long inputs to the rarest class and recalled
the commonest label once in sixteen.

**Two numbers, not one.** The golden score cannot be read alone, because
the served model is fitted on the file it scores. The two that were
actually steered by are 5-fold cross-validated accuracy on those same
pairs, and whether every adversarial probe still passes -- a question and
the same question with a sentence appended to it must decide the same way.
Both are cheap to recompute and both move when SMOOTHING or `_features`
moves. Tuning on the golden number alone tunes nothing.
...
# Smoothing keeps a token this label never produced from vetoing it outright,
# and that is all it is for. Add-one is the textbook value and it is wrong
# for this shape of data: one-line questions of six or seven words, seventy-
# seven labels, and a label's whole evidence is a few hundred counts against
# a smoothing mass of alpha * |vocabulary|. At alpha = 1 the smoothing is the
# larger term, every likelihood flattens toward the prior, and the decision
# becomes the label balance rather than the text -- the commonest label came
# back for questions whose own cue word was sitting in them.
#
# Lowering it trades the other way, and the trade is not free. The cost is
# paid on the probes: an unsteered question and the same question with a
# sentence appended must decide the same, and the smaller alpha is, the more
# a single stray word moves the score. The value below is the one that
# maximised 5-fold cross-validated accuracy (0.800 +/- 0.002, against 0.765
# for add-one) AMONG the settings that keep every adversarial probe passing.
# 0.1 cross-validates a little better and loses a probe by 0.03 nats; that is
# the constraint binding, not noise. Re-tune with both numbers in front of
# you, never with the golden score, which the served model is fitted on.
SMOOTHING = 0.2
MIN_TEXT_CHARS = 8
...
def _features(text: str) -> list[str]:
    """The words, and each adjacent pair of them.

    Single words cannot separate these labels on their own, and the pair
    that proves it is `direct_debit_payment_not_recognised` against
    `extra_charge_on_statement`. On "there is a strange charge on my debit
    statement" the word `debit` argues for the first by four nats and the
    words `charge` and `statement` argue for the second by three and a half:
    whichever wins, wins by a whisker, and a whisker is not a decision. It
    is also not robust -- one word appended to that question, occurring
    twice in the whole corpus, was enough to turn it over. `debit statement`
    as one feature is the thing that was actually being asked about, and
    it carries the case by two and a half nats instead of one.

    Pairs are formed AFTER the stopwords are dropped, so the pair spans
    them: it is `charge statement` that recurs across this client's
    questions, not `charge on`, and a pair of two words that are next to
    each other only because the words between them were dropped is still
    the pair a person would have read.
    """
    words = _tokens(text)
...
class Reasoning:
    ...
    def scores(self, text: str) -> dict[str, float]:
        ...
        present = Counter(t for t in _features(text) if t in self._vocabulary)
        out = {}
        for label in LABELS:
    ...
    def run(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        by_model = self.decide_with_model(text)
        chosen = by_model or baseline
        if chosen not in LABELS:
            raise NotALabel(f"{chosen!r} is not one of the {len(LABELS)} labels")
        return {
            **payload,
            "decision": chosen,
            "decided_by": "model" if by_model is not None else "baseline",
