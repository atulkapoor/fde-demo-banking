...
1. **Who judges**: the evaluation owner -- NOT yet named (the client_readiness gate was waived), so finding this person is an open action this protocol cannot run without -- the client_readiness gate is part of the engagement record that produced this project, not of this project -- plus at least one person who does the work today. Not the builder.
2. **Sample**: fresh items from live data -- never the golden set (the system has seen those 6946 in CI). Size to match the golden set or 30, whichever is larger.
...
- `golden.jsonl`: 6946 cases, sha256 `14a996ceb289dba4`
- `edge_case.jsonl`: 17 cases, sha256 `ce2bfc755cbe1a91`
- `adversarial.jsonl`: 11 cases, sha256 `b0c20f7e3b246255`
- holdout: 3036 cases, sha256 `4081d4fbd2f2caba` -- kept with the engagement, never shipped; fde implement --holdout must be given the file with this digest
